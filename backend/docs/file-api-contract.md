...
파일/버전 조회 API는 인증 사용자를 기준으로 접근을 제어합니다.

- `PRIVATE` 프로젝트는 참여 멤버만 파일/버전 조회가 가능합니다.
- `PUBLIC` 프로젝트는 로그인한 비멤버도 파일 트리, 파일 상세, 파일 검색, 버전 목록, 버전 상세, 버전 타임라인, 버전 diff 조회가 가능합니다.
- 파일 생성, 이름 변경, 이동, 삭제, 현재 내용 저장, 명시적 버전 저장, 버전 복원은 프로젝트 멤버 권한을 기준으로 제한합니다.
- 프로젝트 채팅, presence, 멤버 목록처럼 사용자 활동이나 협업 맥락이 드러나는 정보는 이 문서 범위에서 제외하며, 멤버 접근 정책을 유지합니다.
