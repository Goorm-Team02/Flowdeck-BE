...
class ProjectFileServiceIntegrationTest {
  ...
  void deleteFileAlsoDeletesFileVersions() {
    ...
    projectFileService.deleteFile(project.getPublicId(), file.getId());

    assertThat(projectFileRepository.findById(file.getId())).isEmpty();
    assertThat(fileVersionRepository.findAllByFileOrderByVersionNumberDesc(file)).isEmpty();
  }
  ...
  void deleteFolderAlsoDeletesChildFileVersions() {
    ...
    projectFileService.deleteFile(project.getPublicId(), folder.getId());

    assertThat(projectFileRepository.findById(folder.getId())).isEmpty();
    assertThat(projectFileRepository.findById(file.getId())).isEmpty();
    assertThat(fileVersionRepository.findAllByFileOrderByVersionNumberDesc(file)).isEmpty();
  }
}
